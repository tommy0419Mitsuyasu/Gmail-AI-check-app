...
function initDragAndDrop() {
    ...
    function uploadFile(file) {
        ...
        try {
            ...
            // 希望単価（下限/上限）をフォームデータに追加
            try {
                const minSalaryEl = document.getElementById('minSalaryInput');
                const maxSalaryEl = document.getElementById('maxSalaryInput');
                if (minSalaryEl && minSalaryEl.value !== '') {
                    formData.append('min_salary', String(parseInt(minSalaryEl.value, 10)));
                }
                if (maxSalaryEl && maxSalaryEl.value !== '') {
                    formData.append('max_salary', String(parseInt(maxSalaryEl.value, 10)));
                }
            } catch (e) {
                // 無視（単価入力が無い/数値でない等）
            }
    ...
    try {
        ...
        if (data.status === 'success' && data.matches && data.matches.length > 0) {
            ...
            const projects = data.matches.map(project => ({
                ...project,
                source: 'gmail',
                matched_skills: Array.isArray(skills) ? skills.filter(skill =>
                    (project.description || '').toLowerCase().includes((skill.name || skill).toLowerCase()) ||
                    (project.title || '').toLowerCase().includes((skill.name || skill).toLowerCase())
                ) : [],
                match_count: project.match_count !== undefined ? project.match_count : 0,
                match_percentage: project.match_percentage !== undefined ? project.match_percentage : 0
            }));
            // マッチ率を計算
            projects.forEach(project => {
                // バックエンドからの本格的なスコアが来ていればそれを優先
                if (project.match_percentage === undefined || project.match_percentage === 0) {
                    project.match_count = project.matched_skills.length;
                    project.match_percentage = Math.min(project.match_count * 20, 100); // フォールバック
                }
            });
            // マッチり率の降順でソート（最高マッチの高いものが必ず上に来る）
...
function openGmailWithProject(project) {
    ...
    if (project.gmail_url) {
        console.log('Gmailを開きます (gmail_url):', project.gmail_url);
        window.open(project.gmail_url, '_blank', 'noopener,noreferrer');
        return;
    }
    
    const messageId = project.message_id || project.id;
    if (messageId && String(messageId).length > 5) {
        const url = `https://mail.google.com/mail/u/0/#inbox/${messageId}`;
        console.log('Gmailを開きます (message):', url);
        window.open(url, '_blank', 'noopener,noreferrer');
        return;
    }
    
    if (project.thread_id) {
        const url = `https://mail.google.com/mail/u/0/#all/${project.thread_id}`;
        console.log('Gmailを開きます (thread):', url);
        window.open(url, '_blank', 'noopener,noreferrer');
        return;
    }
    
    if (project.rfc822_message_id) {
        const url = `https://mail.google.com/mail/u/0/#search/rfc822msgid:${encodeURIComponent(project.rfc822_message_id)}`;
        console.log('Gmailを開きます (rfc822msgid):', url);
        window.open(url, '_blank', 'noopener,noreferrer');
        return;
    }
    
    let searchQuery = 'to:sales@artwize.co.jp ';
    const subj = project.subject || project.title || project.name;
    if (subj) searchQuery += `subject:"${subj}" `;
    if (project.client_name) searchQuery += `from:"${project.client_name}" `;
    if (project.location && project.location !== '未記載') searchQuery += `"${project.location}"`;
    const gmailSearchUrl = `https://mail.google.com/mail/u/0/#search/${encodeURIComponent(searchQuery.trim())}`;
    console.log('Gmail検索を開きます (fallback):', gmailSearchUrl);
