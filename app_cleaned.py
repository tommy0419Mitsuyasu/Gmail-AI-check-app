...
def check_gmail_auth():
    ...
    try:
        ...
        # 繝医・繧ｯ繝ｳ縺ｮ譛牙柑譛滄剞繧堤｢ｺ隱搾ｼ・0蛻・ｻ･蜀・↓蛻・ｌ繧句ｴ蜷医・繝ｪ繝輔Ξ繝・す繝･繧定ｩｦ縺ｿ繧具ｼ・        if credentials.expired or (credentials.expiry and 
...
# 繧ｻ繧ｭ繝･繝ｪ繝・ぅ繝倥ャ繝繝ｼ繧定ｿｽ蜉縺吶ｋ繝溘ラ繝ｫ繧ｦ繧ｧ繧｢
@app.after_request
...
def extract_text_from_pdf(file_path: str) -> str:
    ...
    Returns:
        str: 謚ｽ蜃ｺ縺輔ｌ縺溘ユ繧ｭ繧ｹ繝・        
    Raises:
        Exception: 繝輔ぃ繧､繝ｫ縺ｮ隱ｭ縺ｿ霎ｼ縺ｿ繧・ユ繧ｭ繧ｹ繝域歓蜃ｺ縺ｫ螟ｱ謨励＠縺溷ｴ蜷・    """
    try:
...
def analyze_resume(file_path: str) -> Dict:
    ...
    try:
        ...
        try:
            ...
            print("謚ｽ蜃ｺ縺輔ｌ縺溘ユ繧ｭ繧ｹ繝医・蜈磯ｭ100譁・ｭ・", text[:100] + ("..." if len(text) > 100 else ""))
...
def upload_file():
    ...
    try:
        ...
        # 繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺溷ｴ蜷医√い繝・・繝ｭ繝ｼ繝峨＆繧後◆繝輔ぃ繧､繝ｫ繧貞炎髯､
...
def gmail_auth():
    # 譌｢縺ｫ隱崎ｨｼ貂医∩縺ｧ譛牙柑縺ｪ繝医・繧ｯ繝ｳ縺後≠繧句ｴ蜷医・繝励Ο繧ｸ繧ｧ繧ｯ繝井ｸ隕ｧ縺ｫ繝ｪ繝繧､繝ｬ繧ｯ繝・    if 'credentials' in session:
        try:
            creds = Credentials(
    ...
    try:
        ...
        # 隱崎ｨｼURL繧堤函謌撰ｼ井ｸ蠎ｦ縺縺第価隱阪ｒ豎ゅａ繧具ｼ・        auth_url, _ = flow.authorization_url(
            access_type='offline',
...
def gmail_auth_callback():
    ...
    # 繧ｿ繧､繝繧｢繧ｦ繝医メ繧ｧ繝・け・・0蛻・ｻ･蜀・ｼ・    if datetime.now().timestamp() - oauth_flow.get('timestamp', 0) > 1800:
        app.logger.error("OAuth flow timed out")
        session.pop('oauth_flow', None)
        session.modified = True
        flash('繧ｻ繝・す繝ｧ繝ｳ縺ｮ譛牙柑譛滄剞縺悟・繧後∪縺励◆縲ゅｂ縺・ｸ蠎ｦ縺願ｩｦ縺励￥縺縺輔＞縲・, 'error')
        return redirect(url_for('get_projects'))
    ...
    try:
        ...
        # 繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺溷ｴ蜷医・隱崎ｨｼ迥ｶ諷九ｒ繧ｯ繝ｪ繧｢
        session.pop('credentials', None)
        session.pop('oauth_flow', None)
        session.pop('oauth_state', None)
...
def find_matching_engineers(project_requirements: List[Dict[str, Any]], project_context: str = "") -> List[Dict[str, Any]]:
    ...
    Args:
        project_requirements: 繝励Ο繧ｸ繧ｧ繧ｯ繝郁ｦ∽ｻｶ縺ｮ繝ｪ繧ｹ繝医ょ推隕∫ｴ縺ｯ莉･荳九・繧ｭ繝ｼ繧呈戟縺､霎樊嶌:
            - skill: 繧ｹ繧ｭ繝ｫ蜷・(蠢・・
    ...
    for row in cursor.fetchall():
        if row['skill_name']:  # 繧ｹ繧ｭ繝ｫ縺後≠繧句ｴ蜷医・縺ｿ霑ｽ蜉
    ...
    try:
        ...
        return jsonify({
            ...
            'message': str(e)
                ...
                # 繧ｹ繝九・繝・ヨ繧貞叙蠕暦ｼ域悽譁・・蜈磯ｭ200譁・ｭ暦ｼ・                snippet = email_body[:200] + '...' if len(email_body) > 200 else email_body
                ...
                try:
                    ...
                    # 譌･莉倥・繝代・繧ｹ縺ｫ螟ｱ謨励＠縺溷ｴ蜷医・迴ｾ蝨ｨ譎ょ綾繧剃ｽｿ逕ｨ
...
error_messages = {
    ...
    'invalid_file': '繝輔ぃ繧､繝ｫ縺檎ｴ謳阪＠縺ｦ縺・ｋ縺九√し繝昴・繝医＆繧後※縺・↑縺・ｽ｢蠑上〒縺吶・
}
...
def get_gmail_service():
    ...
    try:
        ...
        try:
            ...
            # 繧ｵ繝ｼ繝薙せ菴懈・縺ｫ螟ｱ謨励＠縺溷ｴ蜷医・隱崎ｨｼ諠・ｱ繧堤┌蜉ｹ蛹・            session.pop('credentials', None)
            session.pop('gmail_authenticated', None)
            return None
            
    except Exception as e:
        app.logger.error(f"Error in get_gmail_service: {str(e)}", exc_info=True)
        # 繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺溷ｴ蜷医・隱崎ｨｼ諠・ｱ繧偵け繝ｪ繧｢
        session.pop('credentials', None)
        session.pop('gmail_authenticated', None)
        return None
